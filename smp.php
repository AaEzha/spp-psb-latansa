...
if(!isset($_GET['tipe'])){
	// daftar siswa SMP
?>
<h2>Daftar Santri SMP <small>| <a class="btn btn-xs btn-primary" href="?hal=smp&tipe=input">Input Santri Baru</a></small></h2>
<table class="table" id="tbl">
	...
	<tbody>
		<?php
		$q = mysql_query("select * from siswa where jenis='smp'");
		$no = 1;
		...
		<tr>
			...
			<td class="text-center"><a href="?hal=smp&tipe=input&id=<?php echo $d->idsiswa; ?>" class="btn btn-xs btn-success">ubah</a> <a href="?hal=bayar1&tipe=smp&id=<?php echo $d->idsiswa; ?>" class="btn btn-xs btn-warning">pembayaran 1</a> <a href="?hal=bayar2&tipe=smp&id=<?php echo $d->idsiswa; ?>" class="btn btn-xs btn-danger">pembayaran 2</a></td>
		</tr>
		<?php $no++; } ?>
	</tbody>
</table>
	...
	switch ($tipe) {
		...
		if(isset($_GET['id'])){
			...
			if(isset($_POST['namasiswa'])){
				...
				$jk = $_POST['jk'];
				// input
				mysql_query("insert into siswa(namasiswa,jk,jenis) values('$namasiswa','$jk','smp')") or die('Gagal input');
				eksyen('','?hal=smp');
			}
		}
?>
<h2>Input/Edit Santri SMP <small>| <a class="btn btn-xs btn-primary" href="?hal=smp">Kembali</a></small></h2>
